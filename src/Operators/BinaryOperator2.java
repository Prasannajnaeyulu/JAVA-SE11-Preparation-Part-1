...
public class BinaryOperator2 {
    public static void main(String[] args) {
        ...
        // here s2.intern() looks for string in stringconstantpool if one is available it returns the same reference
        // As we've one in stringconstantpool created by String s1 = "hello" statement above
        // s3 gets the same reference so s1 and s3 will equal
        String s3 = s2.intern();
        if (s1 == s3) {
        ...
        byte testByte = 127;
        testByte += 1;
        System.out.println("testByte = " + testByte); // -128

        // interesting: compiler error: required byte found int
//        byte testByte2 = 127;
